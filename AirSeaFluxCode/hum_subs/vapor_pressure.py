...
def _vp_liquid(meth, T, T_Celsius, P):  # noqa: C901
    if meth in (["HylandWexler", ""]):
        ...
        """
        Psat = (
            np.exp(-7235.424651 / T + 77.34491296 + 5.7113e-3 * T - 8.2 * np.log(T))
            / 100
        )
    elif meth == "Wexler":
        """
        ...
        Psat = (
            np.exp(
                ...
                +
                # This line was corrected from '-' to '+' following the original
                # citation. (HV 20140819). The change makes only negligible
                # difference
...
def _vp_ice(meth, T, T_Celsius: np.ndarray, P):  # noqa: C901
    ...
    if meth == "MartiMauersberger":
        ...
        Psat = (
            np.exp(
                -0.58653696e4 * np.power(T, -1)
                + 0.22241033e2
                + 0.13749042e-1 * T
                - 0.34031775e-4 * np.power(T, 2)
        ...
        Psat = (
            np.exp(
                -0.58666426e4 * np.power(T, -1)
                + 0.2232870244e2
                + 0.139387003e-1 * T
                - 0.34262402e-4 * np.power(T, 2)
                + 0.27040955e-7 * np.power(T, 3)
                + 0.67063522e-1 * np.log(T)
            )
            / 100
        )
        ...
        """
        Psat = np.exp(
            -6024.5282 * np.power(T, -1)
            + 24.721994
            + 1.0613868e-2 * T
            - 1.3198825e-5 * np.power(T, 2)
            - 0.49382577 * np.log(T)
        )
    elif meth == "MurphyKoop":
        """
        ...
        """
        Psat = (
            np.exp(9.550426 - 5723.265 / T + 3.53068 * np.log(T) - 0.00728332 * T) / 100
        )
    else:
