...
class S88:
    ...
    def _wind_iterate(self, ind):
        if self.gust[0] in range(1, 7):
            self.wind[ind] = np.sqrt(
                np.power(np.copy(self.spd[ind]), 2)
                + np.power(
                    get_gust(
                        self.gust[1],
                        self.gust[2],
    ...
    def _update_coolskin_warmlayer(self, ind):
        if self.cskin == 1:
            ...
            #     self.skin)
            if self.skin == "C35":
                self.dter[ind], self.tkt[ind] = cs_C35(
                    np.copy(self.SST[ind]),
                    self.rho[ind],
                    self.Rs[ind],
                    self.Rnl[ind],
                    self.cp[ind],
            ...
            self.dqer[ind] = get_dqer(
                self.dter[ind], self.SST[ind], self.qsea[ind], self.lv[ind]
            )  # [g/kg]
            ...
            if self.wl == 1:
                self.dtwl[ind] = wl_ecmwf(
                    ...
                    self.qsr[ind],
                    np.copy(self.SST[ind]),
                    np.copy(self.skt[ind]),
                    np.copy(self.dter[ind]),
                    self.grav[ind],
                )
    ...
    def _first_guess(self):
        ...
        def dummy_array(val):
            return np.full(self.T.shape, val) * self.msk
        ...
        if self.cskin + self.wl > 0:
            ...
            self.dqer = get_dqer(self.dter, self.SST, self.qsea, self.lv)  # [g/kg]
            self.Rnl = 0.97 * (
                self.Rl - 5.67e-8 * np.power(self.SST - 0.3 * self.cskin, 4)
            )
    ...
    def iterate(self, maxiter=10, tol=None):
        ...
        # specification
        tol = ["all", 0.01, 0.01, 1e-02, 1e-3, 0.1, 0.1] if tol is None else tol
        ...
        self.itera, self.tau, self.sensible, self.latent = [
            dummy_array(x) for x in (-1, 1e99, 1e99, 1e99)
        ]
        ...
        while ii & (it < maxiter):
            ...
            self.ct10n[ind], self.zot[ind] = ctqn_calc(
                "ct",
                self.h_in[1, ind] / self.monob[ind],
                self.cd10n[ind],
                self.usr[ind],
                self.zo[ind],
                self.theta[ind],
            ...
            self.cq10n[ind], self.zoq[ind] = ctqn_calc(
                "cq",
                self.h_in[2, ind] / self.monob[ind],
                self.cd10n[ind],
                self.usr[ind],
                self.zo[ind],
                self.theta[ind],
            ...
            log_vars = [
                np.round(np.nanmedian(getattr(self, V)), R) for V, R in log_vars.items()
            ]
            ...
            if self.cskin + self.wl > 0:
                self.Rnl[ind] = 0.97 * (
                    self.Rl[ind]
                    - 5.67e-8 * np.power(self.SST[ind] + self.dter[ind] * self.cskin, 4)
                )
            # not sure how to handle lapse/potemp
            # well-mixed in potential temperature ...
            self.t10n[ind] = (
                self.theta[ind]
                - self.tlapse[ind] * self.ref10
                - self.tsr[ind]
                / kappa
    ...
    def _flag(self, out=0):
        ...
        if self.cskin == 1:
            miss = np.where(np.isnan(self.msk + self.P + self.Rs + self.Rl), np.nan, 1)
        else:
    ...
    def get_output(self, out_var=None, out=0):
        ...
        self.latent = (
            self.rho * self.lv * (self.usr / self.GFo[2]) * self.qsr * 0.001
        )  # qsr: [g/kg]
        ...
        resAll = pd.DataFrame(data=res.T, index=range(self.nlen), columns=res_vars)  # type:ignore
    ...
    def _class_flag(self):
        ...
        self.flag = np.where(
            ...
            "o",
            np.where(
                ((self.u10n < self.u_lo[1]) | (self.u10n > self.u_hi[1]))
                & (
                    (self.flag != "n")
...
class YT96(S88):
    """Yelland & Taylor 1996 Parametrization"""
...
class LP82(S88):
    """Large & Pond 1982 Parametrization"""
